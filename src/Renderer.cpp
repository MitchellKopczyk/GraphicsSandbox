...
void Renderer::Initalize()
{
	...
	BuildInputLayout();
	ObjModelUno->Load("", Device.Get(), ImmediateContext.Get());
...
{
	HR(D3DCompileFromFile(L"VertexShader.hlsl", 0, 0, "main", "vs_5_0", 0, 0, VertexBlob.GetAddressOf(), 0));
	HR(D3DCompileFromFile(L"PixelShader.hlsl", 0, 0, "main", "ps_5_0", 0, 0, PixelBlob.GetAddressOf(), 0));

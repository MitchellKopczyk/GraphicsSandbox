...
Mesh::Mesh(Model& model, aiMesh& mesh, ID3D11Device* Device)
{
	...
	BuildBuffers(Device, TempVertexStorgae, Indices);
}

void Mesh::BuildBuffers(ID3D11Device* Device, std::vector<Vertex>& Verticies, std::vector<UINT>& Indicies)
{
	D3D11_BUFFER_DESC vertexBufferDesc;
	ZeroMemory(&vertexBufferDesc, sizeof(vertexBufferDesc));
	vertexBufferDesc.Usage = D3D11_USAGE_DEFAULT;
	vertexBufferDesc.ByteWidth = sizeof(Vertex) * Verticies.size();
	...
	vertexBufferData.pSysMem = &Verticies.at(0);
	...
	indexBufferDesc.ByteWidth = sizeof(UINT) * Indicies.size();
	indexBufferDesc.BindFlags = D3D11_BIND_INDEX_BUFFER;
	indexBufferDesc.CPUAccessFlags = 0;
	indexBufferDesc.MiscFlags = 0;

	D3D11_SUBRESOURCE_DATA iinitData;
	iinitData.pSysMem = &Indicies.at(0);
